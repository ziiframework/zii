...
//        $this->assertSame(false, $model->bool_col2);
    ...
    public function testLegalValuesForFindByCondition($modelClassName, $validFilter): void
    {
        ...
        $sql = Customer::getDb()->queryBuilder->build($query);

        $this->assertIsString($sql[0]);
        $this->assertIsArray($sql[1]);
    }
    ...
    public function testLoadRelations(): void
    {
        ...
        Customer::loadRelationsFor($customers, ['orders.items' => static function ($query): void { $query->asArray(false); }], true);

        foreach ($customers as $customer) {
            $this->assertTrue(isset($customer['orders']));
            $this->assertIsArray($customer['orders']);
        ...
        /** @var Customer $customer */
        $customer = Customer::find()->where(['id' => 2])->with(['orders' => static function ($query): void { $query->orderBy(['id' => SORT_ASC]); }])->one();
        $this->assertTrue($customer->orders[0]->id < $customer->orders[1]->id, 'Related models should be sorted by ID in ascending order.');
        $customer->loadRelations(['orders' => static function ($query): void { $query->orderBy(['id' => SORT_DESC]); }]);
