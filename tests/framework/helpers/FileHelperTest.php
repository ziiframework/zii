...
final class FileHelperTest extends TestCase
{
    ...
    public function testCopyDirectoryRecursive(): void
    {
        ...
        $checker = function ($structure, $dstDirName) use (&$checker): void {
    ...
    public function testCopyDirToSubdirOfItself(): void
    {
        ...
        FileHelper::copyDirectory($this->testFilePath . DIRECTORY_SEPARATOR . 'backup', $this->testFilePath . DIRECTORY_SEPARATOR . 'backup' . DIRECTORY_SEPARATOR . 'data');
    }
    ...
    public function testCopyDirToAnotherWithSameName(): void
    {
        ...
        FileHelper::copyDirectory($this->testFilePath . DIRECTORY_SEPARATOR . 'data', $this->testFilePath . DIRECTORY_SEPARATOR . 'backup' . DIRECTORY_SEPARATOR . 'data');
    ...
    public function testCopyDirWithSameName(): void
    {
        ...
        FileHelper::copyDirectory($this->testFilePath . DIRECTORY_SEPARATOR . 'data', $this->testFilePath . DIRECTORY_SEPARATOR . 'data-backup');
    }
    ...
    public function testGetMimeType(): void
    {
        ...
        $this->assertTrue(in_array(FileHelper::getMimeType($file), ['application/json', 'text/plain'], true));
    }
    ...
    public function testNormalizePath(): void
    {
        ...
        $this->assertSame('\\\\server\share\path\file', FileHelper::normalizePath('\\\\server\share\path\file', '\\'));

        // Stream Wrappers should not have the double slashes stripped
        // https://github.com/yiisoft/yii2/issues/17235
        $this->assertSame('ftp://192.168.1.100/test', FileHelper::normalizePath('ftp://192.168.1.100/test/'));
    }
    ...
    public function testChangeOwnership(): void
    {
        ...
        $this->assertSame($currentUserId, fileowner($testFile), 'Expected created test file owner to be current user.');
        $this->assertSame($currentGroupId, filegroup($testFile), 'Expected created test file group to be current group.');
        $this->assertSame('0' . decoct($fileMode), substr(decoct(fileperms($testFile)), -4), 'Expected file mode to be changed.');
        ...
        clearstatcache(true, $testFile);
        $this->assertSame($currentUserId, fileowner($testFile), 'Expected file owner to be unchanged.');
        $this->assertSame($currentGroupId, filegroup($testFile), 'Expected file group to be unchanged.');
        $this->assertSame('0' . decoct($fileMode), substr(decoct(fileperms($testFile)), -4), 'Expected file mode to be changed.');
        ...
        clearstatcache(true, $testFile);
        $this->assertSame($ownership, fileowner($testFile), 'Expected file owner to be changed.');
        $this->assertSame($currentGroupId, filegroup($testFile), 'Expected file group to be unchanged.');
        $this->assertSame('0' . decoct($fileMode), substr(decoct(fileperms($testFile)), -4), 'Expected file mode to be unchanged.');

        // Test user ownership as numeric string (should be treated as integer)
        $ownership = '10002';
        FileHelper::changeOwnership($testFile, $ownership);
        clearstatcache(true, $testFile);
        $this->assertSame((int) $ownership, fileowner($testFile), 'Expected created test file owner to be changed.');
        $this->assertSame($currentGroupId, filegroup($testFile), 'Expected file group to be unchanged.');
        $this->assertSame('0' . decoct($fileMode), substr(decoct(fileperms($testFile)), -4), 'Expected file mode to be unchanged.');
        ...
        FileHelper::changeOwnership($testFile, $ownership);
        clearstatcache(true, $testFile);
        $this->assertSame($ownership, posix_getpwuid(fileowner($testFile))['name'], 'Expected created test file owner to be changed.');
        $this->assertSame($currentGroupId, filegroup($testFile), 'Expected file group to be unchanged.');
        $this->assertSame('0' . decoct($fileMode), substr(decoct(fileperms($testFile)), -4), 'Expected file mode to be unchanged.');
        ...
        FileHelper::changeOwnership($testFile, $ownership);
        clearstatcache(true, $testFile);
        $this->assertSame(substr($ownership, 0, -1), posix_getpwuid(fileowner($testFile))['name'], 'Expected created test file owner to be changed.');
        $this->assertSame($currentGroupId, filegroup($testFile), 'Expected file group to be unchanged.');
        $this->assertSame('0' . decoct($fileMode), substr(decoct(fileperms($testFile)), -4), 'Expected file mode to be unchanged.');
        ...
        FileHelper::changeOwnership($testFile, $ownership);
        clearstatcache(true, $testFile);
        $this->assertSame($ownership[0], fileowner($testFile), 'Expected created test file owner to be changed.');
        $this->assertSame($currentGroupId, filegroup($testFile), 'Expected file group to be unchanged.');
        $this->assertSame('0' . decoct($fileMode), substr(decoct(fileperms($testFile)), -4), 'Expected file mode to be unchanged.');
        ...
        FileHelper::changeOwnership($testFile, $ownership);
        clearstatcache(true, $testFile);
        $this->assertSame((int) $ownership[0], fileowner($testFile), 'Expected created test file owner to be changed.');
        $this->assertSame($currentGroupId, filegroup($testFile), 'Expected file group to be unchanged.');
        $this->assertSame('0' . decoct($fileMode), substr(decoct(fileperms($testFile)), -4), 'Expected file mode to be unchanged.');
        ...
        FileHelper::changeOwnership($testFile, $ownership);
        clearstatcache(true, $testFile);
        $this->assertSame($ownership['user'], posix_getpwuid(fileowner($testFile))['name'], 'Expected created test file owner to be changed.');
        $this->assertSame($currentGroupId, filegroup($testFile), 'Expected file group to be unchanged.');
        $this->assertSame('0' . decoct($fileMode), substr(decoct(fileperms($testFile)), -4), 'Expected file mode to be unchanged.');
        ...
        clearstatcache(true, $testFile);
        $this->assertSame($currentUserId, fileowner($testFile), 'Expected file owner to be unchanged.');
        $this->assertSame((int) substr($ownership, 1), filegroup($testFile), 'Expected created test file group to be changed.');
        $this->assertSame('0' . decoct($fileMode), substr(decoct(fileperms($testFile)), -4), 'Expected file mode to be unchanged.');
        ...
        clearstatcache(true, $testFile);
        $this->assertSame($currentUserId, fileowner($testFile), 'Expected file owner to be unchanged.');
        $this->assertSame(substr($ownership, 1), posix_getgrgid(filegroup($testFile))['name'], 'Expected created test file group to be changed.');
        $this->assertSame('0' . decoct($fileMode), substr(decoct(fileperms($testFile)), -4), 'Expected file mode to be unchanged.');
        ...
        clearstatcache(true, $testFile);
        $this->assertSame($currentUserId, fileowner($testFile), 'Expected file owner to be unchanged.');
        $this->assertSame($ownership['group'], filegroup($testFile), 'Expected created test file group to be changed.');
        $this->assertSame('0' . decoct($fileMode), substr(decoct(fileperms($testFile)), -4), 'Expected file mode to be unchanged.');
        ...
        clearstatcache(true, $testFile);
        $this->assertSame($currentUserId, fileowner($testFile), 'Expected file owner to be unchanged.');
        $this->assertSame((int) $ownership['group'], filegroup($testFile), 'Expected created test file group to be changed.');
        $this->assertSame('0' . decoct($fileMode), substr(decoct(fileperms($testFile)), -4), 'Expected file mode to be unchanged.');
        ...
        clearstatcache(true, $testFile);
        $this->assertSame($currentUserId, fileowner($testFile), 'Expected file owner to be unchanged.');
        $this->assertSame($ownership['group'], posix_getgrgid(filegroup($testFile))['name'], 'Expected created test file group to be changed.');
        $this->assertSame('0' . decoct($fileMode), substr(decoct(fileperms($testFile)), -4), 'Expected file mode to be unchanged.');
        ...
        FileHelper::changeOwnership($testFile, $ownership);
        clearstatcache(true, $testFile);
        $this->assertSame((int) explode(':', $ownership)[0], fileowner($testFile), 'Expected file owner to be changed.');
        $this->assertSame((int) explode(':', $ownership)[1], filegroup($testFile), 'Expected created test file group to be changed.');
        $this->assertSame('0' . decoct($fileMode), substr(decoct(fileperms($testFile)), -4), 'Expected file mode to be unchanged.');
        ...
        FileHelper::changeOwnership($testFile, $ownership);
        clearstatcache(true, $testFile);
        $this->assertSame(explode(':', $ownership)[0], posix_getpwuid(fileowner($testFile))['name'], 'Expected file owner to be changed.');
        $this->assertSame(explode(':', $ownership)[1], posix_getgrgid(filegroup($testFile))['name'], 'Expected created test file group to be changed.');
        $this->assertSame('0' . decoct($fileMode), substr(decoct(fileperms($testFile)), -4), 'Expected file mode to be unchanged.');
        ...
        FileHelper::changeOwnership($testFile, $ownership);
        clearstatcache(true, $testFile);
        $this->assertSame($ownership[0], fileowner($testFile), 'Expected file owner to be changed.');
        $this->assertSame($ownership[1], filegroup($testFile), 'Expected created test file group to be changed.');
        $this->assertSame('0' . decoct($fileMode), substr(decoct(fileperms($testFile)), -4), 'Expected file mode to be unchanged.');
        ...
        FileHelper::changeOwnership($testFile, $ownership);
        clearstatcache(true, $testFile);
        $this->assertSame((int) $ownership[0], fileowner($testFile), 'Expected file owner to be changed.');
        $this->assertSame((int) $ownership[1], filegroup($testFile), 'Expected created test file group to be changed.');
        $this->assertSame('0' . decoct($fileMode), substr(decoct(fileperms($testFile)), -4), 'Expected file mode to be unchanged.');
        ...
        FileHelper::changeOwnership($testFile, $ownership);
        clearstatcache(true, $testFile);
        $this->assertSame($ownership[0], posix_getpwuid(fileowner($testFile))['name'], 'Expected file owner to be changed.');
        $this->assertSame($ownership[1], posix_getgrgid(filegroup($testFile))['name'], 'Expected created test file group to be changed.');
        $this->assertSame('0' . decoct($fileMode), substr(decoct(fileperms($testFile)), -4), 'Expected file mode to be unchanged.');
        ...
        FileHelper::changeOwnership($testFile, $ownership);
        clearstatcache(true, $testFile);
        $this->assertSame($ownership['user'], fileowner($testFile), 'Expected file owner to be changed.');
        $this->assertSame($ownership['group'], filegroup($testFile), 'Expected created test file group to be changed.');
        $this->assertSame('0' . decoct($fileMode), substr(decoct(fileperms($testFile)), -4), 'Expected file mode to be unchanged.');
        ...
        FileHelper::changeOwnership($testFile, $ownership);
        clearstatcache(true, $testFile);
        $this->assertSame((int) $ownership['user'], fileowner($testFile), 'Expected file owner to be changed.');
        $this->assertSame((int) $ownership['group'], filegroup($testFile), 'Expected created test file group to be changed.');
        $this->assertSame('0' . decoct($fileMode), substr(decoct(fileperms($testFile)), -4), 'Expected file mode to be unchanged.');
        ...
        FileHelper::changeOwnership($testFile, $ownership);
        clearstatcache(true, $testFile);
        $this->assertSame($ownership['user'], posix_getpwuid(fileowner($testFile))['name'], 'Expected file owner to be changed.');
        $this->assertSame($ownership['group'], posix_getgrgid(filegroup($testFile))['name'], 'Expected created test file group to be changed.');
        $this->assertSame('0' . decoct($fileMode), substr(decoct(fileperms($testFile)), -4), 'Expected file mode to be unchanged.');
        ...
        clearstatcache(true, $testFile);
        $this->assertSame(explode(':', $ownership)[0], fileowner($testFile), 'Expected created test file owner to be changed.');
        $this->assertSame(explode(':', $ownership)[1], filegroup($testFile), 'Expected file group to be unchanged.');
        $this->assertSame('0' . decoct($fileMode), substr(decoct(fileperms($testFile)), -4), 'Expected created test file mode to be changed.');
    }
    ...
    public function testChangeOwnershipNonExistingUser(): void
    {
        ...
        try {
            ...
            $this->assertSame('chown(): Unable to find uid for non_existing_user', $e->getMessage());
        }
    }
